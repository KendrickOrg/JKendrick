...
public class Main {
	...
	private static void xchartExample(double[][] results, double step, double last)
		...
		double[] yDataS = new double[results[0].length];
		double[] yDataI = new double[results[1].length];
		double[] yDataR = new double[results[2].length];
		for (int i = 0; i< (int) Math.ceil(last / step); ++i) {
			xData[i]= i * step;
			yDataS[i]= results[0][i];
			yDataI[i]= results[1][i];
			yDataR[i]= results[2][i];
		}
		...
		// Create Chart
				...
				// Series
				chart.addSeries("S", xData, yDataS);
				chart.addSeries("I", xData, yDataI);
				chart.addSeries("R", xData, yDataR);
